...
class Bisektion():
    ...
    **Bisektion**: Implements the bisection method for root finding.
    ...
    def calc_separator(self, longest_text: str, header: str):
        """
        **calc_separator**: Calculates padding for formatted output.

        Parameters:
            longest_text (str): The longest text to align.
            header (str): The header text.

        Returns:
            tuple: Left padding, right padding, and total length (int).
        """
        total_length = len(longest_text)  # Total length of the separator
    ...
    def main_loop(self):
        ...
        **main_loop**: Executes the bisection method until the desired accuracy is achieved.
        ...
        left_side_length, right_side_length, total_length = self.calc_separator(
            longest_text=f'fa: {self.fa}, fb: {self.fb}, fc: {self.fc}', header=f' Iteration: {iteration} '
        )
        ...
        if self.result:
            left_side_length, right_side_length, total_length = self.calc_separator(
                longest_text=f'fa: {self.fa}, fb: {self.fb}, fc: {self.fc}', header=f' Result '
            )
            print("=" * left_side_length, "Result", "=" * right_side_length)
            for key, value in self.result.items():
                result_text = f'{key} {value}'
                left_padding, right_padding, _ = self.calc_separator(longest_text="=" * total_length, header=result_text)
...
if __name__ == "__main__":
    ...
    bisektion.accuracy = 0.001  # Desired accuracy (e.g. 1e-50)
